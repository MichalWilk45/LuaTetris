    ...
    for y = 1, #self.activeBlock do
        for x = 1, #self.activeBlock[y] do
            if self.activeBlock[y][x] == 1 then
                ...
                -- Sprawdzanie, czy klocek dotknął dolnej krawędzi planszy
                if boardY >= boardHeight -1 then  -- Dolna granica planszy
                    return true
                end

                -- Sprawdzanie, czy klocek koliduje z innymi zablokowanymi klockami (tylko na dole)
                if self.activeBlock[y + 1] and self.activeBlock[y + 1][x] == 1 then
                    local nextY = boardY + 1
                    print("nextY", nextY)
                    if self.board[nextY] and self.board[nextY][boardX] == 1 then
                        return true
                    end
                end
            end
